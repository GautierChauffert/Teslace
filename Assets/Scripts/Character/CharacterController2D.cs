...
public class CharacterController2D : Resettable
{
    ...
    private void FixedUpdate()
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            {
                m_Grounded = true;
                if (m_animator != null) m_animator.SetBool("isJumping", false);
                if (!wasGrounded)
                    OnLandEvent.Invoke();
            }
        }
    ...
    public void Move(float move, bool crouch, bool jump)
    {
        ...
        if (m_Grounded && jump)
        {
            ...
            m_Grounded = false;
            if (m_animator != null) m_animator.SetBool("isJumping", true);
            if (m_Rigidbody2D.velocity.y > 4f)
                return;
